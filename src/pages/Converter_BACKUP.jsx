...
function Converter() {
    ...
    const loadAllData = async () => {
        try {
            ...
            if (!session) {
                console.warn("[Converter] WARNING: No Supabase session found via getSession(). Checking auth context user:", user);
                if (user?.id) console.log("[Converter] Context User ID:", user.id);
            } else {
                console.log("[Converter] Active Supabase Session ID:", session.user.id);
            }
    ...
    const restoreOriginalParams = () => {
        ...
        modelObject.traverse((child) => {
            if (child.isMesh && child.userData.initialMat) {
                if (Array.isArray(child.userData.initialMat)) {
                    child.material = child.userData.initialMat.map(m => m.clone());
                } else {
        ...
        modelObject.traverse((child) => {
            if (child.isMesh && child.material && !child.userData.initialMat) {
                ...
                if (Array.isArray(child.material)) {
                    child.userData.initialMat = child.material.map(m => m.clone());
                } else {
    ...
    // ==========================================
    // ROBUST SAVE FLOW (3-Step Reliability)
    // ==========================================
    const confirmSaveToProject = async () => {
        // 1. Validate User & Data
        if (!saveData.name) {
            toast({ title: "Nombre Requerido", description: "Por favor nombra tu modelo", variant: "destructive" });
            return;
        }
        if (!user) {
            toast({ title: "Error de Sesión", description: "Debes iniciar sesión para guardar.", variant: "destructive" });
            return;
        }

        setShowSaveDialog(false);
        setLoading(true);
        setUploadStatus("Iniciando motor de guardado...");
        setProgress(5);

        let activeProjectId = saveData.projectId;
        ...
        try {
            ...
            if (!activeProjectId) {
                setUploadStatus("Paso 1/3: Asegurando Proyecto...");
                console.log("[Reliability] Step 1/3: Ensuring Project Exists...");
                if (projects.length > 0) {
                    activeProjectId = projects[0].id;
                } else {
                    console.log("No projects found, creating default...");
                    const newProj = await projectsService.create({
            ...
            console.log(`[Reliability] Active Project ID: ${activeProjectId}`);
            setProgress(30);

            // STEP 2: Generate Content (GLB)
            setUploadStatus("Paso 2/3: Generando binarios 3D...");
            const blob = await generateGLB();
            if (!blob || blob.size === 0) throw new Error("Generación de GLB fallida (0 bytes).");

            // Create proper File object
            const fileName = `${saveData.name.replace(/[^a-z0-9]/gi, '_').toLowerCase()}.glb`;
            const file = new File([blob], fileName, { type: 'model/gltf-binary' });
            setProgress(50);

            // STEP 3: Upload & Register (Atomic Operation in DataService)
            setUploadStatus("Paso 3/3: Subiendo a Nube Segura...");

            // This handles Storage Upload + DB Insert with Retries internally
            const asset = await modelsService.uploadAssetAndCreate(file, activeProjectId, user.id);
            ...
            toast({
                ...
                description: "Tu modelo ha sido asegurado en la librería."
            });

            // Optional: Best effort thumbnail (Non-blocking)
            try {
                if (rendererRef.current && sceneRef.current && cameraRef.current) {
                    rendererRef.current.render(sceneRef.current, cameraRef.current);
                    rendererRef.current.domElement.toBlob(blob => {
                        if (blob) {
                            const thumbFile = new File([blob], fileName.replace('.glb', '.png'), { type: 'image/png' });
                            modelsService.uploadAssetAndCreate(thumbFile, activeProjectId, user.id).catch(e => console.warn("Thumb failed", e));
                        }
                    });
                }
            ...
            if (error.message && error.message.includes('Auth')) msg = "Sesión inválida.";
            if (error.message && error.message.includes('Network')) msg = "Fallo de red.";
            if (error.message && error.message.includes('policy')) msg = "Permisos denegados (RLS).";
    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-[#0B0F14] text-white flex flex-col h-screen overflow-hidden">
                ...
                <header className="border-b border-[#29B6F6]/20 bg-[#151B23] p-4 shrink-0">
                    <div className="container mx-auto flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            {/* Tu Librería Button */}
                            <Button onClick={() => setShowLibrary(!showLibrary)} variant="secondary" size="sm" className="hidden md:flex text-gray-300 hover:text-white text-xs gap-1 border border-white/5 ml-2">
                                <BookOpen className="h-3 w-3 mr-1" /> Tu Librería
                            </Button>

                            {/* STATUS LED */}
                            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#0B0F14] border border-white/10 ml-2 shadow-inner">
                                <div className={`h-2.5 w-2.5 rounded-full ${user ? 'bg-[#00FF94] shadow-[0_0_8px_#00FF94] animate-pulse' : 'bg-[#FF0055] shadow-[0_0_8px_#FF0055]'}`} />
                                <span className={`text-[10px] font-mono tracking-wider ${user ? 'text-[#00FF94]' : 'text-[#FF0055]'}`}>
                                    {user ? 'ONLINE' : 'OFFLINE'}
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="hidden md:flex gap-2">
                            {activeTab === 'image' ? (
                                ...
                                <>
                                    ...
                                    <Button onClick={() => setShowLibrary(!showLibrary)} variant="ghost" className="md:hidden text-white">
                ...
                {/* FUTURISTIC ERROR BANNER */}
                {
                    !user && (
                        <div className="w-full bg-[#FF0055]/10 border-b border-[#FF0055]/30 backdrop-blur-md p-2 flex items-center justify-center gap-3 animate-in slide-in-from-top-10">
                            <div className="h-2 w-2 rounded-sm bg-[#FF0055] rotate-45" />
                            <span className="text-[#FF0055] font-mono text-xs tracking-[0.2em] uppercase font-bold">
                                Sistema Desconectado: Modo Emergencia Activado
                            </span>
                            <div className="h-2 w-2 rounded-sm bg-[#FF0055] rotate-45" />
                        </div>
                    )
