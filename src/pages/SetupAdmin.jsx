...
import { CheckCircle, XCircle, AlertCircle, Database, Server, Settings, Loader2, ArrowLeft } from 'lucide-react';
...
function SetupAdmin() {
  ...
  const checkStatus = async () => {
    ...
    try {
      ...
      newStatus.storage = hasModelsBucket ? true : 'warning';
    } catch (e) {
  ...
  return (
    <>
      <Helmet>
        <title>{t('admin.title')} - VISOR-X</title>
      </Helmet>
            ...
            <div className="bg-[#151B23] p-6 rounded-lg border border-[#29B6F6]/20">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-semibold">{t('admin.envConfig')}</h2>
                </div>
                {isSupabaseConfigured() ? (
                  <span className="text-green-400 flex items-center gap-1"><CheckCircle className="h-4 w-4" /> {t('admin.configured')}</span>
                ) : (
                  <span className="text-red-400 flex items-center gap-1"><XCircle className="h-4 w-4" /> {t('admin.missing')}</span>
                )}
              </div>
            ...
            <div className="bg-[#151B23] p-6 rounded-lg border border-[#29B6F6]/20">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-semibold">{t('admin.serviceHealth')}</h2>
                </div>
                <Button
                  onClick={checkStatus}
                  size="sm"
                ...
                >
                  {status.checking ? <Loader2 className="h-4 w-4 animate-spin" /> : t('admin.runDiagnostics')}
                </Button>
              </div>

              <div className="space-y-4">
                <div className="flex items-center justify-between p-3 bg-[#0B0F14] rounded">
                  <span>{t('admin.dbConnection')}</span>
